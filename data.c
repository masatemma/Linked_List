...
void 
stringProcess(char converted_grade1in[], int figures_num){
    int after_decimal = 0, decimal_point;

    for(int i=0; i<figures_num; i++){
        if(converted_grade1in[i] == '.'){
            after_decimal = 1;
            decimal_point = i;
        }
        if(after_decimal){
            for(int j=figures_num - 1; j > decimal_point; j--){
                if(converted_grade1in[j] != '0'){
                    if(figures_num - 1 != j){
                        converted_grade1in[j + 1] = '\0';
                        break;
                    }                
                }
                else if(j == decimal_point + 1 && converted_grade1in[j] == '0'){
                    converted_grade1in[decimal_point] = '\0';
                    break;
                }
            }
            break;
